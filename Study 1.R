...
coef <- c(514.5024,-20.3716,-10.8582,-3.9191,15.7089,10.7096,-10.8876,8.5165,9.7752,-2.8759,-2.7069,5.5208,4.8477,-3.3229)
coef <- as.matrix(coef, nrow = length(coef), ncol = 1)

## Generate fixed-effects part
IV <- data[,c(2,53,54,73,80,135:138,140,142,189,237)] # locations of predictors with non-zero coefficients
...
f2 <- as.formula(paste("MAT ~" , paste(n[!n %in% "MAT"], collapse = " + "), " + (1|IDSCHOOL)"))
...
for (k in 1:NN){
  ...
  b_0j <- rnorm(230, mean = 0, sd = 37.31) # school-level random effects
  Rij <- rnorm(8116, mean = 0, sd = 60.88) # residuals
  
  ## Repeat each school-level random effect for the number of students in the corresponding school
  b <- c()
  for (j in 1:length(sch_num)){
    b <- c(b, rep(b_0j[j], sch_num[j]))
  }
  ...
  ### Extract random effects (estimated random intercept)
  random_effects <- ranef(glmm_model)
  level2.residuals <- as.numeric(random_effects$IDSCHOOL[,"(Intercept)"])
  
  ### Save the number of students for each school in the training data
  sch_num2 <- as.numeric(table(train$IDSCHOOL))
  
  ### Repeat each school-level random effect for the number of students in the corresponding school
  level2.residuals_2 <- c()
  for (j in 1:length(sch_num2)){
    level2.residuals_2 <- c(level2.residuals_2, rep(level2.residuals[j], sch_num2[j]))
  }
  
  ### Extract independent and dependent variables
  x <- model.matrix(f, data=train)[,-1]
  y <- train$MAT- level2.residuals_2 # extract the random intercept from the original dependent variable
  ...
  ### Repeat each school-level random effect for the number of students in the corresponding school
  level2.residuals_3 <- c()
  for (j in 1:length(sch_num3)){
    level2.residuals_3 <- c(level2.residuals_3, rep(level2.residuals[j], sch_num3[j]))
  }
  
  ## Test LassoML on the test data
  xx <- model.matrix(f,data=test)[,-1]
  yy <- test$MAT - level2.residuals_3 # extract the random intercept from the original dependent variable
  ...
  for (l in seq_along(lambdas)) {
    ...
    for (i in seq_along(folds)) {
      ...
      fit <- tryCatch({
        glmmLasso(fix = f, rnd = list(IDSCHOOL=~1), data = train_data_cv, 
  ...
  glmm.fit1 <- glmmLasso(fix = f, rnd = list(IDSCHOOL=~1), data = train, family=gaussian(link="identity"), lambda = lambda.1se)
  glmm.fit2 <- glmmLasso(fix = f, rnd = list(IDSCHOOL=~1), data = train, family=gaussian(link="identity"), lambda = lambda.min)
  ...
  PQL.fit <- glmmPQL(f, random = ~1|IDSCHOOL, family = gaussian, data = train)
  ...
  PQL_significant <- glmmPQL(pql.form, random = ~1|IDSCHOOL,
